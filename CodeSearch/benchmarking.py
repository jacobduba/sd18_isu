...
def convert_examples_to_features(js, tokenizer, args):
    ...
    code = ' '.join(js['code_tokens']) if type(
        js['code_tokens']) is list else ' '.join(js['code_tokens'].split())
    ...
    nl = ' '.join(js['docstring_tokens']) if type(
        js['docstring_tokens']) is list else ' '.join(js['doc'].split())
...
def train(args, model, tokenizer):
    ...
    for idx in range(args.num_train_epochs):
        ...
        results = evaluate(args, model, tokenizer,
                           args.eval_data_file, eval_when_training=True)
...
def evaluate(args, model, tokenizer, file_name, eval_when_training=False):
    ...
    for example in query_dataset.examples:
        ...
        nl_strings.append(example.docstring)
