...
if not client.api_key:
    raise ValueError(
        "OpenRouter API key is missing. Set it in your environment variables."
    )
...
def evaluate_snippet(user_input: str, snippet: str, retries=3):
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if not response_text:
                print(
                    f"Warning: No response from LLM (Attempt {attempt + 1}). Retrying..."
                )
    ...
    print(
        "Final Warning: No response from LLM after multiple attempts. Returning score 0."
    )
...
def rank_snippets(user_input: str, snippets: list):
    ...
    print("\n--- Ranking Snippets ---")
    scored_snippets = [
        (snippet, evaluate_snippet(user_input, snippet)) for snippet in snippets
    ]
