...
def post_process(code):
    code = code.replace("<string", "<STR_LIT").replace(
        "<number", "<NUM_LIT").replace("<char", "<CHAR_LIT")
    code = code.replace("<NUM_LIT>", "0").replace(
        "<STR_LIT>", "").replace("<CHAR_LIT>", "")
...
def tokenize(item):
    ...
    source_tokens = ["<s>", "<decoder-only>", "</s>"] + \
        source_tokens[-(max_length-3):]
...
def main():
    ...
    if args.lang == "python":
        ...
        eos_ids = [tokenizer.convert_tokens_to_ids('Ġ;'), tokenizer.convert_tokens_to_ids(
    ...
    if args.do_train:
        ...
        train_features = convert_examples_to_features(
            train_examples, tokenizer, args, pool, stage='train')
        ...
        logger.info("  Step per epoch = %d", len(
            train_examples)//args.train_batch_size)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for idx, batch in enumerate(train_dataloader):
                ...
                if (idx+1) % 100 == 0:
                    logger.info("epoch {} step {} loss {}".format(
                        epoch, idx+1, round(np.mean(losses[-100:]), 4)))
            ...
            if args.do_eval:
                ...
                eval_examples = read_examples(
                    args.dev_filename.replace(".json", ".txt"))
                ...
                logger.info("  %s = %s " %
                            ("loss", round(np.mean(dev_losses), 4)))
                logger.info("  %s = %s " % ("Acc", str(dev_acc)))
                logger.info("  %s = %s " %
                            ("Edit sim", str(round(edit_sim/total, 2))))
                logger.info("  "+"*"*20)
                if dev_acc > best_acc:
